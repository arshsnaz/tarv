...
export const submitAccessRequest = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const { error } = await resend.emails.send({
      ...
      to: toEmail,
      replyTo: data.email,
      subject: `⚡ New Access Request: ${data.name} (${data.company})`,
      html: `
        <!DOCTYPE html>
        <html>
        <head>
                ...
                <table width="100%" border="0" cellspacing="0" cellpadding="0" style="max-width:600px; background-color:#121215; border:1px solid #27272a; border-radius:16px; overflow:hidden; box-shadow:0 20px 25px -5px rgba(0, 0, 0, 0.5);">
                  ...
                  <tr>
                    <td style="padding:28px 32px 12px 32px;">
                      ...
                      <p style="margin:8px 0 0 0; font-size:14px; color:#a1a1aa; line-height:1.5;">
                        An enterprise client has submitted a request for early access to the TARV MEP platform.
                      </p>
                    </td>
                  </tr>

                  <!-- Applicant Details Card Grid -->
                  <tr>
                      ...
                      <table width="100%" border="0" cellspacing="0" cellpadding="0" style="background-color:#18181b; border:1px solid #27272a; border-radius:12px; padding:20px; table-layout:fixed;">
                        ...
                        <tr>
                          <td style="padding-bottom:14px;">
                            ...
                            <span style="font-size:15px; font-weight:600; color:#ffffff; display:block; margin-top:3px;">${escapeHtml(data.company)}</span>
                          </td>
                          <td style="padding-bottom:14px;">
                            <span style="font-size:11px; font-weight:600; color:#71717a; text-transform:uppercase; letter-spacing:0.8px; display:block;">Company Size</span>
                            <span style="font-size:14px; font-weight:500; color:#f4f4f5; display:block; margin-top:3px;">${escapeHtml(data.companySize)}</span>
                          </td>
                        </tr>

                        <tr>
                          <td style="padding-bottom:6px;">
                            <span style="font-size:11px; font-weight:600; color:#71717a; text-transform:uppercase; letter-spacing:0.8px; display:block;">Country</span>
                            <span style="font-size:14px; font-weight:500; color:#f4f4f5; display:block; margin-top:3px;">${escapeHtml(data.country)}</span>
                          </td>
                          <td style="padding-bottom:6px;">
                            <span style="font-size:11px; font-weight:600; color:#71717a; text-transform:uppercase; letter-spacing:0.8px; display:block;">Primary System Focus</span>
                            <span style="font-size:14px; font-weight:500; color:#38bdf8; display:block; margin-top:3px;">${escapeHtml(data.system)}</span>
                          </td>
                        </tr>
                  ...
                  <tr>
                    <td style="padding:0 32px 28px 32px;">
                      <span style="font-size:12px; font-weight:600; color:#a1a1aa; text-transform:uppercase; letter-spacing:0.8px; display:block; margin-bottom:8px;">Workflow Bottlenecks & Notes</span>
                      <div style="background-color:#18181b; border-left:3px solid #00f2fe; border-radius:4px 8px 8px 4px; padding:16px; font-size:14px; color:#e4e4e7; line-height:1.6; font-style:italic;">
                  ...
                  <tr>
                    <td style="padding:0 32px 32px 32px;" align="center">
                      <a href="mailto:${escapeHtml(data.email)}?subject=${encodeURIComponent(`Re: TARV Enterprise Beta Access — ${data.company}`)}" style="display:inline-block; background:linear-gradient(90deg, #00f2fe 0%, #4facfe 100%); color:#09090b; font-size:14px; font-weight:700; padding:12px 28px; border-radius:9999px; text-decoration:none; box-shadow:0 4px 14px rgba(0, 242, 254, 0.3);">
